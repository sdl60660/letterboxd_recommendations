...
def create_user_test_train_sets(user_data_df, test_users, user_train_test_split=0.7, random_seed=RANDOM_SEED):
  output_set = []

  for user_id in test_users:
    user_data = user_data_df[user_data_df['user_id'] == user_id]

    user_train_data = user_data.sample(frac=user_train_test_split, random_state=random_seed)
...
def eval_fold_in_user(user_data_set, model):
  ...
  predictions = [Prediction(uid=username, iid=x['movie_id'], r_ui=test_key[x['movie_id']]['rating_val'], est=x['predicted_rating'], details=None) for x in results if x['movie_id'] in test_key]
...
def eval_param_set_fold_in(base_model, params, training_dataset, user_data_sets):
    ...
    if sum_preds == 0:
        ...
        return {'fold_in_rmse': float('nan'), 'fold_in_precision@k': float('nan'), 'fold_in_recall@k': float('nan')}
...
def eval_fold_in(df, base_model, param_set_df, num_test_users=1000):
  ...
  for i, params in enumerate(tqdm(params_set, desc="Param sets")):
    param_metrics = eval_param_set_fold_in(base_model, params, training_dataset, user_data_sets)
