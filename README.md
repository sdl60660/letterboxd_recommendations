...
config = {
    ...
    "MONGO_DOCKER_URL":"mongodb://mongodb:27017/" # Optional, only if you're running the app with Docker
}

tmdb_key = "123" # Your TMDB API key
```
...
If you'd like to run the web server locally using Docker, you can do so by running `docker-compose up` from the root directory. This will start up a local Redis instance, a local MongoDB instance, the worker and the main app. After running the scrapers as descibed above, you can then navigate to `localhost:8000` to access the app.
