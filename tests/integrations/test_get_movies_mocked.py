...
async def _run_once(mongo_db, monkeypatch):
    ...
    monkeypatch.setattr(
        get_movies,
        "cffi_async_session",
        lambda **kwargs: FakeSession({url: body}),
    )
...
def test_get_movies_inserts_document(mongo_db, monkeypatch, event_loop):
    event_loop.run_until_complete(_run_once(mongo_db, monkeypatch))
